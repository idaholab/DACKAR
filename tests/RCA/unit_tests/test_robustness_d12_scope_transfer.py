...
_scenario_shared = Path(__file__).resolve().parents[1] / "scenario" / "shared"
for _p in (str(_repo_root), str(_rca_root), str(_scenario_shared)):
...
# Fixture paths
# ---------------------------------------------------------------------------
TC7_FIXTURE_DIR = Path(__file__).resolve().parents[1] / "scenario" / "test_case_7" / "fixtures"
