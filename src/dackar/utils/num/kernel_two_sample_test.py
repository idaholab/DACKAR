...
def MMD2u_UCB(K, m, alpha=0.05):
    """
        Uniform convergence bound for MMD2u
    """
...
def MMD2b_UCB(K, m, alpha=0.05):
    """
        Uniform convergence bound for MMD2b
    """
    assert alpha > 0 and alpha < 1, f'alpha should be within [0,1], but got {alpha}'
    Kxy = K[:m, m:]
    maxKxy = np.max((0,np.max(Kxy)))
...
def kernel_two_sample_test(X, Y, kernel_function='rbf', iterations=2000,
    ...
    valid values for kernel_function are:
    ...
    if thin is not None:
        try:
            ...
            print(f'Try to perform thin calculation, but get non-integer value for "thin: {thin}"')
            thin = None
    if thin is not None:
        X = X[0::thin, :]
        Y = Y[0::thin, :]
    ...
    if verbose:
        print(f"UCB bound for MMD^2_u is {ucb}")
    ...
    if verbose:
        ...
        if p_value < alpha:
            print(f"Deviaition from null hyothesis is statistically significant at test level {alpha}, reject null hyothesis, \n \
...
def chebyshevTesting(X, Y, kernel_function='rbf', iterations=2000,
    ...
    """
        Chebyshev Testing using Chebyshev's Inequality
    """
    accept = False
...
def chebyshevTesting_precomputed_mmd(mmd2u, mmd2u_null, alpha=0.01):
    ...
    if diff < nu * sigma:
        accept = True
        print(f"Chebyshev Inequality holds: {diff} < {nu*sigma}")
    else:
        accept = False
        print(f"Chebyshev Inequality does not hold: {diff} > {nu*sigma}")
    return accept
