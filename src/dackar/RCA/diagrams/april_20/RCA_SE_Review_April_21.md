...
| **FMEA handling spec Phase 1 (new)** | Added `doc_parsers/fmea_normalizer.py` with explicit format profiles (AIAG/MIL-STD/IEC/nuclear/auto), derivation rules (`rpn`, MIL-STD occurrence and severity mappings, effect fallback, cause/effect heuristic split), and per-field quality tagging (`present_native`, `derived`, `nlp_inferred`, `missing_*`). `doc_parsers/fmeaParser.py` now performs profile-aware column resolution and normalization by default; `kg/kg_ingest_fmea_workflow.py` writes FMEA ingestion quality into `kg_provenance_latest` and emits `fmea_ingestion_report.json` (default). Added schema: `schemas/fmea_ingestion_report.json`; validator registration in `validation/schema_validator.py`; tests: `unit_tests/test_fmea_normalizer.py`, `unit_tests/test_fmea_parser_normalization.py`. |
