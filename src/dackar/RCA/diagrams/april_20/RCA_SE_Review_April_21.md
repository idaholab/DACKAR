...
**Date**: April 21, 2026 · **Sprint 1 applied**: April 21, 2026 · **Sprint 2 applied**: April 21, 2026 · **Option B applied**: April 21, 2026 · **Sprint 5 applied**: April 21, 2026
...
### 3.1 Strictly Sequential Pipeline with No Recovery Paths (CRITICAL — NEW)
...
### 3.2 Silent Failure Propagation — No Mid-Pipeline Error Protocol (HIGH — NEW)
...
### 3.3 KG Population is a Silent Prerequisite with No Runtime Governance (CRITICAL — NEW)
...
### 4.5 Two Independent Allen Computations Without Coordination (HIGH — NEW)
...
**Recommendation**: Store the Stage B Allen pre-filter results in `kg_context.out_of_boundary_anomalies[]` with `allen_relation` field (already proposed). Have Stage C read this field to pre-annotate any TSKR pattern whose component has `allen_relation == "FOLLOWS"` with `temporal_contradiction_flag: True` before scoring begins.

---

### 4.6 Symptom Matching Not Implemented (MEDIUM — FROM WORKFLOW GAP)

**Finding**: Per `RCA_workflow_april_2.md` §6: "event.symptom_signature.symptom_types and anomaly_pattern fields are not currently used in scoring." The `kg_context.failure_modes[].expected_symptoms` and `expected_anomaly_pattern` fields exist in the schema but contribute zero to candidate scores.

**Consequence**: A failure mode that perfectly predicts `gradual_drift` and the component shows `gradual_drift` receives no scoring bonus. A failure mode that predicts `rapid_spike` receives no penalty when the component shows `gradual_drift`. Symptom matching is a direct, objective causal signal that is free — the data exists in both the event artifact and the KG.

**Recommendation**: Add symptom matching as a sub-component of the temporal score (or a sixth scoring dimension). Binary match: `expected_anomaly_pattern` in event's `symptom_signature.anomaly_patterns` → +0.20 bonus; mismatch → −0.10 penalty. Cap to prevent single-dimension dominance.

---

## 5. Data Quality, Traceability, and Provenance

### 5.1 Source Tier Metadata Not Yet Tagged in Chroma (HIGH — ⚠️ PARTIALLY FIXED Sprint 2)
...
**Remaining gap**: Evidence retriever does not yet populate `best_source_tier` in per-candidate summaries — authority weighting in Stage F is dormant until that field is added.
...
### 5.2 `component_ids` Filter in Chroma Is a No-Op (HIGH — CONFIRMED FROM WORKFLOW)
...
### 5.4 FM-to-CR Matching Depends on NER Quality (MEDIUM — NEW)
...
**Recommendation**: (1) Log FM-to-CR match failures at Stage C (how many CRs could not be mapped to any failure mode). (2) Add `unmatched_cr_count` to `tskr_patterns` metadata. (3) Surface `analyst_attention_flags["high_cr_match_failure_rate"]` when > 30% of CRs are unmatched.

---

### 5.5 Allen Interval Endpoint Completeness — Open vs Closed Intervals (MEDIUM — NEW)
...
**Recommendation**: Add `interval_type: "closed" | "open" | "half_open_start" | "half_open_end"` to anomaly schema. Default to `closed`. Document in `RCA_Data_Management_Strategy.md` the interval convention used by the Anomaly Detection System.

---

### 5.6 Historical Event Pool at Stage D Still Empty (HIGH — NEW)
...
### 5.7 Two-Path Document Ingestion Not Yet Routed (MEDIUM — NEW)
...
**A1 — No cross-artifact timestamp consistency** (MEDIUM): `telemetry.window.end` is not verified to be ≥ `event.timestamp_start`. A telemetry summary from the wrong time window passes silently.
...
**A3 — `output_dir` writability not verified at entry** (LOW): Stage I will fail mid-pipeline if `output_dir` is not writable. Stage A should verify writeability and abort early with a clear error rather than failing at persistence time.
...
**C4 — Tone discretization vocabulary undefined** (HIGH — NEW): TSKR's fundamental input is discretized state intervals (Tones). The vocabulary for nuclear plant parameters — what threshold and duration makes a "lube oil low" Tone versus a transient — has not been defined. Without this vocabulary, TSKR's interval classification is sensitive to arbitrary threshold choices. This is called out explicitly in `RCA_workflow_april_2.md` §14 as "the most important next step." Until it is defined and validated, Stage C's output carries undocumented calibration uncertainty.
...
**D4 — `review_alternative_gap` uses only composite delta** (MEDIUM): The review alternative rescue condition checks whether a filtered candidate's composite score is within `review_alternative_gap` (0.10) of the single passing candidate. It does not check `evidence_posture`. A candidate that is `evidence_posture: "contradicted"` should not be rescued as a review alternative regardless of score proximity — a contradicted candidate needs analyst attention, not elevation to the review set.

---

### 6.5 Stage E — Additional Gaps

**E1 — `component_ids` filter no-op** (see §5.2 — ✅ Partial fix Sprint 4): Post-filter already exists in `chroma_store.py`; `LOGGER.warning` now emitted when active and `component_filter_mode: "post_filter"` added to `evidence_bundle.provenance`. True index-level filtering remains medium-term work.
...
**E3 — No evidence retrieval for `out_of_boundary_anomalies`** (HIGH — NEW): Components in `out_of_boundary_anomalies` (sensors with preceding anomalies, especially those with `not_in_kg: True`) are not included in Stage E query plans. These are exactly the components that the analyst most needs evidence about — they are KG coverage gaps. Stage E should include one targeted query per `out_of_boundary_anomalies` component to help the analyst assess whether the gap matters.

**E4 — Contradicting evidence retrieval not structurally specified** (MEDIUM — NEW): Stage E builds "contradiction queries" per candidate, but the spec does not define what makes a retrieved snippet a contradiction. The keyword-based classifier uses terms like "not consistent with," "ruled out," "no evidence of" — but these are absence-of-evidence phrasings, not structural contradictions. A WO finding that confirms a different root cause for the same symptom is a stronger structural contradiction than a snippet that says "no fouling found." The classifier does not distinguish these.
...
**F3 — `review_alternative_gap` on composite only** (see D4 — MEDIUM)

---

### 6.7 Stage H — Additional Gaps

**H1 — Top-k selection ignores `review_required` flag** (HIGH — NEW): Stage H selects the top-5 candidates by composite score for synthesis. If a candidate outside the top-5 has `evidence_posture: "contradicted"` or `temporal_contradiction_flag: True`, it is silently dropped. An engineer reviewing a later RCA that asks "was hypothesis X considered?" will find no record of it in the rca_card. The top-k selection must retain all candidates with `review_required: True` regardless of rank.

**H2 — Evidence not balanced across candidates in synthesis** (MEDIUM — NEW): The fallback synthesizer builds the `evidence[]` block in the rca_card from the top supporting snippets for the primary hypothesis. Snippets that support alternative candidates or contradict the primary candidate are not systematically included. The rca_card can present a one-sided evidence picture, understating the case for alternatives.
...
**I3 — Chroma archive failure not caught** (MEDIUM — NEW): Stage I is responsible for archiving the run-scoped Chroma collection. If the archive step fails (I/O error, permission issue), the pipeline reports success, but the Chroma data is lost. Any post-run evidence re-inspection is impossible. Stage I should treat Chroma archive failure as a pipeline error, not a warning.
...
**J3 — `next_step` routing not wired to analyst workflow** (MEDIUM): The `run_manifest.next_step` field has three values: `writeback | analyst_review | remediation`. In the current implementation, `writeback_ready` is always False, making `next_step` always `analyst_review`. Even when this is fixed, the `next_step` value is never consumed by any downstream system — it is documentation only. An actual workflow integration (email, CMMS ticket, analyst queue entry) is absent.
...
| S4 | `component_ids` filter is a no-op (silent) | HIGH | ⚠️ Partial fix Sprint 4 — post-filter exists + warning logged + provenance flag; true index-level filter is medium-term |
...
| S8 | LLM hallucination gate is warning not error | HIGH | Open — see §6.7 H4 |
...
### 8.1 Contributing Causes Not Representable (CRITICAL — APRIL 20 C2)

Reiterated for regulatory emphasis: INPO AP-923 and IAEA TECDOC-1756 require **contributing causes** as a distinct structured output from alternatives. The current `rca_card` schema has no `contributing_causes[]` array. A contributing cause (necessary but insufficient) is architecturally collapsed into the alternatives block, which is defined as "incorrect" hypotheses. This will cause rca_cards to fail technical review.

### 8.2 Safety Function Impact Never Reaches rca_card (CRITICAL — APRIL 20 C5)

`kg_context.safety_functions[]` is computed by Stage B and partially used in TSKR, but never propagates to `rca_card.recommended_actions[].priority` or `analyst_attention_flags`. An RCA card submitted for a safety-significant event with no safety significance annotation will not pass licensing review.

### 8.3 Barrier Analysis Completely Absent (HIGH — WORKFLOW GAP 3)

No barrier concept exists in the KG schema, candidate scoring, or rca_card output. INPO AP-923 and NRC RIS 2006-13 explicitly require identification of which defense-in-depth barriers failed and which held. This is a structural gap: the KG does not model barriers, so no data can flow from barrier analysis to the rca_card.

### 8.4 Risk Significance Score Not Derived (HIGH — WORKFLOW GAP 4)

`affected_safety_functions` on each candidate is never converted to a `risk_significance_scalar`. "Core Cooling" and "Instrument Air" both appear in `safety_functions[]` but carry identical scoring weight. PRA integration is deferred (per §10 deferred list), but a heuristic safety-function-to-risk-tier mapping is achievable now without PRA data.
...
### 8.7 INPO AP-913 Completeness Criteria Not Mapped (MEDIUM — NEW)

INPO AP-913 defines minimum completeness criteria for a CAP-ready RCA (root cause identified, direct cause identified, contributing causes identified, extent of condition assessed, effectiveness review criteria defined). The `run_manifest` has `writeback_ready` as a binary flag but no AP-913 completeness checklist. An rca_card that passes current validation could still be incomplete per AP-913 and require a return trip from the CAP manager.

**Recommendation**: Add `ap913_completeness` block to `run_manifest`:
```json
{
...
| NH3 | Two independent Allen computations without coordination | §4.5 |
...
| NH7 | `component_ids` filter no-op (confirmed) | §5.2 |
...
| NH11 | Top-k selection ignores `review_required` flag | §6.7 H1 |
| NH12 | LLM hallucination gate is warning, not error (confirmed) | §6.7 H4 |

### New Medium Findings (correctness and data quality)

| ID | Finding | Section |
|----|---------|---------|
| NM1 | Input artifacts have no currency guarantee | §3.4 |
| NM2 | No event scoping check | §3.5 |
| NM3 | Governance weight not failure-mode-category-specific | §4.3 |
| NM4 | Scoring weights unempirical, no sum constraint | §4.4 |
| NM5 | Symptom matching not implemented | §4.6 |
| NM6 | FM-to-CR matching depends on NER quality | §5.4 |
| NM7 | Allen interval endpoint completeness | §5.5 |
| NM8 | Two-path ingestion not routed | §5.7 |
| NM9 | Tone discretization vocabulary undefined | §6.3 C4 |
| NM10 | `review_alternative_gap` on composite only | §6.4 D4 |
| NM11 | Contradicting evidence retrieval not structurally specified | §6.5 E4 |
| NM12 | Evidence not balanced across candidates in synthesis | §6.7 H2 |
| NM13 | Chroma archive failure not caught | §6.8 I3 |
| NM14 | `next_step` not wired to analyst workflow | §6.9 J3 |
| NM15 | INPO AP-913 completeness criteria not mapped | §8.7 |
...
| C4 | Action priority not from safety significance | Open | Related to §8.2 |
| C5 | Safety function never flows to rca_card | Open | Related to §8.2 |
...
*(NM8 two-path ingestion routing and NH3 Allen coordination handshake deferred to Sprint 6)*
...
*Sprint 4 closed: J2 ✅ (is_run_complete + load on FileArtifactStore), S5/E2/NH8 ✅ (BM25 warning + provenance field), S4/E1/NH7 ⚠️ (partial — warning + provenance; post-filter confirmed existing) — 2 fully closed, 1 partial. Remaining open: 35 findings*
*Sprint 5 closed: H4 ✅ (analyst override primary_diff audit trail), H6 ✅ (posture_warning on all action rows), NC5 ✅ (run_context.json + run_manifest.json schemas + CORE_ARTIFACTS) — 3 fully closed. Remaining open: 32 findings*
