...
### Anchor Demonstration Scenario
The showcase must be built around **one component, one outage, one recommendation.** The narrative arc should follow this structure:
...
This single traceable story is the centerpiece of the first stakeholder meeting. All other capabilities are supporting context.
...
### Prediction
- Output: probability of emergent work, estimated delay, confidence tier
...
### Causality
- Identify and score CR → WO → Activity chains
...
Impact score = P(emergent event) × estimated delay (hours) × criticality multiplier

Where criticality multiplier = 1.0 for non-critical path, 2.0–3.0 for near-critical, 5.0 for critical path (exact values to be calibrated against historical data in WBS 9.2).
